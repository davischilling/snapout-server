...
describe('FindRecipeByIdController', () => {
  ...
  let findRecipeByIdService: jest.Mock
  let sut: FindRecipeByIdController
  ...
  beforeAll(async () => {
    ...
    findRecipeByIdService = jest.fn()
    findRecipeByIdService.mockResolvedValue(recipe)
  })

  beforeEach(() => {
    sut = new FindRecipeByIdController(findRecipeByIdService)
  })
  ...
  it('should call findRecipeByIdService with correct params', async () => {
    await sut.handle(id)

    expect(findRecipeByIdService).toHaveBeenCalledWith(id)
    expect(findRecipeByIdService).toHaveBeenCalledTimes(1)
  })

  it('should return 401 if findRecipeByIdService fails', async () => {
    findRecipeByIdService.mockRejectedValueOnce(new ServiceError('service_error'))
  ...
  it('should return 200 if findRecipeByIdService succeeds', async () => {
    const httpResponse = await sut.handle(id)
